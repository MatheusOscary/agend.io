...
const loja_POST = {
    ...
    required: ["nome", "rua", "numero", "cidade", "estado", "cep", "telefone", "email", "login", "senha", "tipo_pessoa"],
    if: {
...
router.post('/', validarjson(loja_POST), async (req, res) => {
    const { nome, rua, numero, cidade, estado, cep, telefone, email, login, senha, tipo_pessoa, cnpj, cpf } = req.body;
    
    try {
        const result = await LojaController.insert(nome, rua, numero, cidade, estado, cep, telefone, email, login, senha, tipo_pessoa, cnpj, cpf);
